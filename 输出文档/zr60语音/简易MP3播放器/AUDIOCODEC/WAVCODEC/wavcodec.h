...
typedef struct FileInfo FIL; 
/********************************************************
 *Resources Interchange File Formant(RIFF)
 *RIFF构成单位为chunk
 *每个chunk=块标志符(4Bytes)+数据大小(4Bytes)+数据
 *若块标志符为RIFF或LIST，则RIFF chunk或LIST chunk内部包含chunk
 *WAV格式类型为WAVE
 *********************************************************/
//RIFF块
typedef __packed struct
{
    u8 ChunkID[4];		   	//chunk id;这里固定为"RIFF",即0X46464952
    u32 ChunkSize ;		   	//集合大小;文件总大小-8
    u8 Format[4];	   			//格式;WAVE,即0X45564157
}ChunkRIFF ;
//fmt块
typedef __packed struct
{
    u8 ChunkID[4];		   	//chunk id;这里固定为"fmt ",即0X20746D66
...
typedef __packed struct 
{
    u8 ChunkID[4];		   	//chunk id;这里固定为"fact",即0X74636166;
    u32 ChunkSize ;		   	//子集合大小(不包括ID和Size);这里为:4.
...
typedef __packed struct 
{
    u8 ChunkID[4];		   	//chunk id;这里固定为"data",即0X5453494C
