...
#include "config.h"
#include "deviceio.h"
#include "sdio_sdcard.h" //存储设备驱动头文件
/*★★★★★★★★★★★★★★★★★★★★★★★★
 ...
 if(0==nsec) return 0;
  ...
  switch(Dev_No)
  {
   case 0:
          ...
          {
           while(SD_ReadDisk(buffer,addr+i,1));
           buffer+=512;
          }
          break;
  // case 1:
  ...
  {
   case 0:
          while(SD_ReadDisk(buffer,addr,nsec));
   //case 1:
 ...
 if(0==nsec) return 0;
  ...
  switch(Dev_No)
  {
   case 0:
          for(i=0;i<nsec;i++)
          {
           while(SD_WriteDisk(buffer,addr+i,1));
           buffer+=512;
          }
		  break;
   //case 1:
...
//		  break;
  ...
  {
   case 0:
          while(SD_WriteDisk(buffer,addr,nsec));
		  break;
   //case 1:
...
UINT8 znFAT_Device_Clear_nSector(UINT32 nsec,UINT32 addr)
{
  ...
  switch(Dev_No)
  {
   case 0:
          for(i=0;i<nsec;i++)
          {
           while(SD_WriteDisk(znFAT_Buffer,addr+i,1));
          }
          break;
   //case 1:
   //       for(i=0;i<nsec;i++)
