...
// 
// if(oem_code<GET_PGM_WORD(&(oem_uni[0][0]))) return ERR_FAIL;
// if(oem_code>GET_PGM_WORD(&(oem_uni[MAX_UNI_INDEX-1][0]))) return ERR_FAIL; //如果输入的oem_code不是表的范围内，则直接返回

// while(low<=high) //当前查找区间[low..high]非空
// {
//  mid=low+(high-low)/2;

//  if(oem_code==GET_PGM_WORD(&(oem_uni[mid][0])))
//  {
//   *uni_code=GET_PGM_WORD(&(oem_uni[mid][1]));

//   return ERR_SUCC; //查找成功返回
//  }

//  if(GET_PGM_WORD(&(oem_uni[mid][0]))>oem_code)
//  {
...
{  
    ...
    if(oem_code<0x80)*uni_code=oem_code; //ASCII码直接不用转换
    else
	{
		 ...
		 for(n=16;n;n--)
		 {
		     ...
		     if(oem_code==t[0])break;
			 if(oem_code>t[0])low=temp;
			 else        high=temp;
		 }
		 *uni_code=n?t[1]:0;
		 if(*uni_code==0)return ERR_FAIL;
	}	
     return ERR_SUCC;	
}
