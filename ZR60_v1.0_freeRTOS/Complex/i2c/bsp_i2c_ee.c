...
uint8_t hym8563(void) 
{
  //uint16_t i;
	//uint8_t write_buf[12] ={0x00,0x13,0x17,0x22,0x11,0x17};
  //uint8_t read_buf[EEPROM_SIZE];
  
/*-----------------------------------------------------------------------------------*/  

	/*
	if (i2c_CheckDevice(0xA2) == 0)
	{
		printf("检测到hym8563\n");
		return 1;
	}
	...
	/*
	if (i2c_master_reg8_send(0xA2, 0x02, write_buf, 4) == 0)
	{
		  printf("写入hym8563 S M H D成功\n");
		
	}
	
	if (i2c_master_reg8_send(0xA2, 0x07, write_buf+4, 2) == 0)
	{
		  printf("写入hym8563 MON YEAER成功\n");
