...
void SPI_FLASH_SectorErase(u32 SectorAddr)
{
  ...
  //SPI_FLASH_WaitForWriteEnd();//20180703 by liujian
}
...
/*******************************************************************************
* Function Name  : SPI_FLASH_WrInProgStatus
* Description    :  write  opertaion has completed?
*                  
* Input          : None
* Output         : None
* Return         : WIP标志无效 -- 0，可操作
				   WIP标志有效 -- 1，不可操作
				   
*******************************************************************************/
u8 SPI_FLASH_WrInProgStatus(void)
{
  u8 FLASH_Status = 0U;
  ...
  SPI_FLASH_SendByte(W25X_ReadStatusReg);
    ...
    and put the value of the status register in FLASH_Status variable */
  FLASH_Status = SPI_FLASH_SendByte(Dummy_Byte);	 
  FLASH_Status = (FLASH_Status & WIP_Flag); /* Write in progress */
  SPI_FLASH_CS_HIGH();
  return FLASH_Status;
}
...
//BUSY:忙标记位(1,忙;0,空闲)
//默认:0x00
u8 W25QXX_ReadSR(u8 StatusReg)   
{  
	u8 byte=0;   
	SPI_FLASH_CS_LOW();                            //使能器件   
	SPI_FLASH_SendByte(StatusReg);    //发送读取状态寄存器命令    
