...
uint64_t DHCPfineTimer = 0;
uint64_t DHCPcoarseTimer = 0;
...
void LwIP_Init(void)
{
...
#ifdef SERIAL_DEBUG
		//USART_PRINTF_S("\n  Static IP address   \n");
		//USART_PRINTF_IP("IP: %d.%d.%d.%d\n",IP_ADDR0,IP_ADDR1,IP_ADDR2,IP_ADDR3);
		//USART_PRINTF_IP("NETMASK: %d.%d.%d.%d\n",NETMASK_ADDR0,NETMASK_ADDR1,NETMASK_ADDR2,NETMASK_ADDR3);
		//USART_PRINTF_IP("Gateway: %d.%d.%d.%d\n",GW_ADDR0,GW_ADDR1,GW_ADDR2,GW_ADDR3);
#endif /* SERIAL_DEBUG */
#endif /* USE_DHCP */
  }
  else
  {
	...
	/* Create tcp_ip stack thread *///使用freeRTOS+lwip时调用，创建mbox
  ...
  switch (DHCP_state)
  {
		...
		case DHCP_WAIT_ADDRESS:
		{
			...
			if (IPaddress!=0) 
			{
				...
				dhcp_stop(&gnetif);
#ifdef SERIAL_DEBUG
				printf("\n【7】 IP address assigned by a DHCP server  √\n");
