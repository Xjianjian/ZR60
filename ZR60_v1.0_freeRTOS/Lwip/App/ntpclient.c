...
*******************************************************/
struct udp_pcb *ntpclient_upcb;//数据收发UDP控制块
...
{
	Sentpclient_dw_TskTimer = NTPCLIENT_TSK_PERIOD - 18000U*5/NTPCLIENT_SCHEDULING_CYCLE;
...
uint8 ntpclient_connect(struct ip_addr SrcIPaddr,struct ip_addr DestIPaddr,uint16 udp_LocalPort,uint16 udp_RemotePort)
{
	...
	if (NULL != ntpclient_upcb)
	{
		...
		if(err == ERR_OK)
		{
			...
			udp_recv(ntpclient_upcb,ntpclient_Rcv_callback,NULL);
			Le_u_Ok = 1U;
			USART_PRINTF_S("Start the ntp server application");
		}
		else
		{
	...
	if(p !=NULL)
	{
		...
		if((p->len==NTP_PKT_LEN) &&(upcb->remote_port == NTPCLIENT_REMOTE_PORT))
		{
			...
			if(((((char*)p->payload)[0]& NTP_MODE_MASK) == NTP_MODE_SERVER)|| ((((char*)p->payload)[0] & NTP_MODE_MASK) == NTP_MODE_BROADCAST))
			{
				...
				SeTransmit_dw_Timestamp = (((uint32_t)(((char*)p->payload)[TRANSMIT_TS_OFFSET])  << 24) | \
										  ((uint32_t)(((char*)p->payload)[TRANSMIT_TS_OFFSET+1]) << 16) | \
										  ((uint32_t)(((char*)p->payload)[TRANSMIT_TS_OFFSET+2]) << 8 ) | \
										  (uint32_t)(((char*)p->payload)[TRANSMIT_TS_OFFSET+3]));
			}
