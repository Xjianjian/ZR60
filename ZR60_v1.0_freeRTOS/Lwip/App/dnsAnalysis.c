...
static uint8  SednsAnalysis_u_UpdateFlag;//ip更新标志，便于实时更新ip
...
void TskdnsAnalysis_MainFunction(void)
{
	...
	if(GET_PHY_LINK_STATUS() && (DHCP_ADDRESS_ASSIGNED == DNS_DHCP_STATE))/* Get Ethernet link status*/
	{
		if((SednsAnalysis_w_Timer >= DNS_ANALYSIS_TASK_PERIOD) || (SednsAnalysis_u_UpdateFlag == 1U))
		{
			SednsAnalysis_w_Timer = 0U;
			SednsAnalysis_u_UpdateFlag = 0U;//ip更新标志
			for(Le_u_i = 0U;Le_u_i < domainNum;Le_u_i++)
			{
				if(SednsAnalysis_u_UpdateHost[Le_u_i] == 1U)
				{
					USART_PRINTF_D("%d DNS域名开始解析\n",Le_u_i);
					dns_gethostbyname(domain[Le_u_i].name,&IPaddr[Le_u_i],&dnsAnalysis_DNSfound,&Se_u_arg[Le_u_i]);//域名解析	
				}
			}
		}
		else
...
static void dnsAnalysis_DNSfound(const char *name, struct ip_addr *ipaddr, void *arg)
{
	...
	if(ipaddr != NULL)
	{
		if(ipaddr->addr == 0)
		{
			...
			USART_PRINTF_D("\r\nErrorLogging：%d DNS域名解析失败,ip地址:0.0.0.0 ",Le_u_obj);
		}
		else
		{
			...
			USART_PRINTF_D("%d DNS域名解析成功,ip地址: ",Le_u_obj);
			USART_PRINTF_IP("%u.%u.%u.%u\n",(uint8_t)SednsAnalysis_dw_DestIPaddr[Le_u_obj].addr, \
	...
	{
		SednsAnalysis_u_UpdateHost[Le_u_obj] = 1U;
		USART_PRINTF_D("\r\nErrorLogging：%d DNS域名解析失败",Le_u_obj);
	}
}
...
void SetdnsAnalysis_ipUpdate(uint8 index)
{
	...
	SednsAnalysis_u_UpdateFlag = 1U;//ip更新标志
}
